...
with tab_flags:
    ...
    year = st.selectbox('Year', range(2022, 2050))
    ...
    df = df[(df['begin_datetime_mpt'] == date)]
    df["begin_datetime_mpt"] = pd.to_datetime(df["begin_datetime_mpt"], format="%Y/%m/%d")
    df["dateModified"] = pd.to_datetime(df["dateModified"], format="%Y/%m/%d")
